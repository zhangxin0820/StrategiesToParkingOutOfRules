...
<body class="body-main">
	<div class="containt-fluid logo-container">
		...
		<%
			//显示操作
			int temp_car_id = Integer.parseInt(request.getParameter("id").trim());
			//System.out.println(temp_sub_id);
			List<Subincident> resultInfor = SubincidentDao.getPersonCar(temp_car_id);
		...
		<div class="containt-fluid" align="center">
			<form Action="" method="post">
				...
				<input type="hidden" id="temp_id1" value="<%=temp_car_id%>" />

				<!-- <input class="btn btn-primary add-btn pull-right" type="button"
					onclick="window.location.href='addparking_sub.jsp'" value="增加新事件" /> -->
				<input class="btn btn-primary add-btn pull-right" type="button"
				...
				<table class="containt-fluid table table-striped" width="100%"
					...
					<tr align="center" height="30"
						...
						<%
							List<Subincident> list = SubincidentDao.getPersonCar(temp_car_id);
					...
					<tr align="center" height="30">
						...
						<%-- <td><a class="link-a btn btn-primary"
							href="showParking_personInfo.jsp?id=<%=sub_id%>&stateId=<%=temp_car_id%>">人物信息</a></td> --%>
				...
				<%
					if (pageCount != 0) {
						if (pageNow != 1) {
							out.println(" <a href=showParking_subInfo.jsp?pageNow=1>" + "&id=" + temp_car_id + "【首页】</a> ");
							out.println(" <a href=showParking_subInfo.jsp?pageNow=" + (pageNow - 1) + "&id=" + temp_car_id
									+ "> 上一页</a> ");
						}
						if (pageNow != pageCount) {
							out.println(" <a href=showParking_subInfo.jsp?pageNow=" + (pageNow + 1) + "&id=" + temp_car_id
									+ "> 下一页</a>");
							out.println(" <a href=showParking_subInfo.jsp?pageNow=" + pageCount + "&id=" + temp_car_id
									+ "> 【尾页】</a>");
						}
						//显示当前页的前三页与后三页
						for (i = ((pageNow - 3) > 0 ? (pageNow - 3) : 1); i <= (pageCount < (pageNow + 3)
								...
								: (pageNow + 3)); i++) {
							out.println(
									"<a href=showParking_subInfo.jsp?pageNow=" + i + "&id=" + temp_car_id + ">[" + i + "]</a>");
						}
					}
					out.print("第");
				%>
		...
		<script type="text/javascript">
			...
			function jump(maxPage) {
				...
				if (pages_int > 0 && pages_int <= maxPage_int) {
					window.location.href = "showParking_carInfo.jsp?pageNow="
							+ pages + "&id=" + tempid1;
					return true;
				} else {
					alert("您输入的页码有误！");
