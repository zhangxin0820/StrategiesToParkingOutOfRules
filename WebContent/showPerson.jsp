...
<body class="body-main">
	<div class="containt-fluid logo-container">
		...
		<%
			...
			List<Parkingperson> resultInfor = ParkingpersonDao.getAll();
			...
			<form Action="" method="post">
				...
				<table class="containt-fluid table table-striped" width="100%"
					...
					<tr align="center" height="30"
						...
						<%
							List<Parkingperson> list = ParkingpersonDao.getAll();
				...
				<%
					if (pageCount != 0) {
						if (pageNow != 1) {
							out.println(" <a   href=showPerson.jsp?pageNow=1>" + "【首页】</a> ");
							out.println(" <a   href=showPerson.jsp?pageNow=" + (pageNow - 1) + "> 上一页</a> ");
						}
						if (pageNow != pageCount) {
							out.println(" <a   href=showPerson.jsp?pageNow=" + (pageNow + 1) + "> 下一页</a>");
							out.println(" <a   href=showPerson.jsp?pageNow=" + pageCount + "> 【尾页】</a>");
						}
						//显示当前页的前三页与后三页
						for (i = ((pageNow - 3) > 0 ? (pageNow - 3) : 1); i <= (pageCount < (pageNow + 3)
								? pageCount
								: (pageNow + 3)); i++) {
							out.println(" <a   href=showPerson.jsp?pageNow=" + i + ">[" + i + "]</a>");
						}
					}
					out.print("第");
				%>
		...
		<script type="text/javascript">
			...
			function jump(maxPage) {
				...
				if (pages_int > 0 && pages_int <= maxPage_int) {
					//alert("bbbb"+pages)
					window.location.href = "showPerson.jsp?pageNow="
							+ pages;
					return true;
				} else {
					alert("您输入的页码有误！");
