...
<body class="body-main">
	<div class="containt-fluid logo-container">
		...
		<%
			//显示操作
			int temp_have_car_id = Integer.parseInt(request.getParameter("id").trim());
			int stateId = Integer.parseInt(request.getParameter("stateId").trim());
			List<Parkingperson> resultInfor = ParkingpersonDao.getAll(temp_have_car_id);
		...
		<div class="containt-fluid" align="center">
			<form Action="" method="post">
				...
				<input class="btn btn-primary add-btn pull-right" type="button"
					onclick="window.location.href='addparking_person.jsp?id=<%=temp_have_car_id%>&stateId=<%=stateId%>'"
					value="增加人物信息" /> <input
					class="btn btn-primary add-btn pull-right" type="button"
					onclick="window.location.href='showParking_subInfo.jsp?id=<%=stateId%>'"
					value="返回车辆信息页面" />
				<table class="containt-fluid table table-striped" width="100%"
					...
					<tr align="center" height="30"
						...
						<%
							List<Parkingperson> list = ParkingpersonDao.getAll(temp_have_car_id);
					...
					<tr align="center" height="30">
						...
						<td><a class="link-a btn btn-primary"
							href="showParking_personDetailed.jsp?id=<%=person_id%>&stateId=<%=stateId%>&haveCarId=<%=temp_have_car_id%>">人物详细信息</a></td>
				...
				<%
					if (pageCount != 0) {
						if (pageNow != 1) {
							out.println(" <a   href=showParking_personInfo.jsp?pageNow=1>" + "&id=" + temp_have_car_id
									+ "&stateId=" + stateId + "【首页】</a> ");
							out.println(" <a   href=showParking_personInfo.jsp?pageNow=" + (pageNow - 1) + "&id="
									+ temp_have_car_id + "&stateId=" + stateId + "> 上一页</a> ");
						}
						if (pageNow != pageCount) {
							out.println(" <a   href=showParking_personInfo.jsp?pageNow=" + (pageNow + 1) + "&id="
									+ temp_have_car_id + "&stateId=" + stateId + "> 下一页</a>");
							out.println(" <a   href=showParking_personInfo.jsp?pageNow=" + pageCount + "&id=" + temp_have_car_id
									+ "&stateId=" + stateId + "> 【尾页】</a>");
						}
						//显示当前页的前三页与后三页
						for (i = ((pageNow - 3) > 0 ? (pageNow - 3) : 1); i <= (pageCount < (pageNow + 3)
								? pageCount
								: (pageNow + 3)); i++) {
							out.println(" <a   href=showParking_personInfo.jsp?pageNow=" + i + "&id=" + temp_have_car_id
									+ "&stateId=" + stateId + ">[" + i + "]</a>");
						}
					}
					out.print("第");
		...
		<script type="text/javascript">
			...
			function jump(maxPage) {
				...
				if (pages_int > 0 && pages_int <= maxPage_int) {
					//alert("bbbb"+pages)
					window.location.href = "showParking_personInfo.jsp?pageNow="
							+ pages + "&id=" + tempid2;
					return true;
				} else {
					alert("您输入的页码有误！");
