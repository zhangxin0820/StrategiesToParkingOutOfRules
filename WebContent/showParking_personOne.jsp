...
<body class="body-main">
	<div class="containt-fluid logo-container">
		...
		<%
			...
			List<Parkingperson> resultInfor = ParkingpersonDao.getOne(temp_person_id);
		...
		<div class="containt-fluid" align="center">
			<form Action="" method="post">
				...
				<input class="btn btn-primary add-btn pull-right" type="button"
					onclick="window.location.href='addparking_person.jsp?id=<%=temp_person_id%>&stateId=<%=stateId%>'"
					value="增加人物信息" /> <input
				...
				<table class="containt-fluid table table-striped" width="100%"
					...
					<tr align="center" height="30"
						...
						<%
							List<Parkingperson> list = ParkingpersonDao.getOne(temp_person_id);
					...
					<tr align="center" height="30">
						...
						<td><a class="link-a btn btn-primary"
							href="showParking_personOneDetailed.jsp?id=<%=person_id%>&stateId=<%=stateId%>">人物详细信息</a></td>
				...
				<%
					if (pageCount != 0) {
						if (pageNow != 1) {
							out.println(
									" <a   href=showParking_personOne.jsp?pageNow=1>" + "&id=" + temp_person_id + "&stateId=" + stateId + "【首页】</a> ");
							out.println(" <a   href=showParking_personOne.jsp?pageNow=" + (pageNow - 1) + "> 上一页</a> ");
						}
						if (pageNow != pageCount) {
							out.println(" <a   href=showParking_personOne.jsp?pageNow=" + (pageNow + 1) + "&id="
									+ temp_person_id + "&stateId=" + stateId + "> 下一页</a>");
							out.println(" <a   href=showParking_personOne.jsp?pageNow=" + pageCount + "&id=" + temp_person_id
						...
						for (i = ((pageNow - 3) > 0 ? (pageNow - 3) : 1); i <= (pageCount < (pageNow + 3)
								? pageCount
								: (pageNow + 3)); i++) {
							out.println(" <a   href=showParking_personOne.jsp?pageNow=" + i + "&id=" + temp_person_id + "&stateId=" + stateId + ">["
									+ i + "]</a>");
						}
					}
					out.print("第");
		...
		<script type="text/javascript">
			...
			function jump(maxPage) {
				...
				if (pages_int > 0 && pages_int <= maxPage_int) {
					//alert("bbbb"+pages)
					window.location.href = "showParking_personOne.jsp?pageNow="
							+ pages + "&id=" + tempid2;
					return true;
				} else {
					alert("您输入的页码有误！");
