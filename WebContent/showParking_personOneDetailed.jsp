	...
	<div class="containt-fluid logo-container">
		...
		<div class="containt-fluid" align="center">
			<form Action="" method="post">
				...
				<input type="hidden" id="temp_id2" value="<%=temp_person_id%>" /> <input
					type="hidden" id="temp_stateId" value="<%=stateId%>" /> <input
					class="btn btn-primary add-btn pull-right" type="button"
					onclick="window.location.href='showParking_personOne.jsp?id=<%=temp_person_id%>&stateId=<%=stateId%>'"
					value="返回人物信息" /> <input
					class="btn btn-primary add-btn pull-right" type="button"
				...
				<%
					if (pageCount != 0) {
						if (pageNow != 1) {
							out.println(
									" <a   href=showParking_personOneDetailed.jsp?pageNow=1>" + "&id=" + temp_person_id + "【首页】</a> ");
							out.println(" <a   href=showParking_personOneDetailed.jsp?pageNow=" + (pageNow - 1) + "> 上一页</a> ");
						}
						if (pageNow != pageCount) {
							out.println(" <a   href=showParking_personOneDetailed.jsp?pageNow=" + (pageNow + 1) + "&id="
									+ temp_person_id + "> 下一页</a>");
							out.println(" <a   href=showParking_personOneDetailed.jsp?pageNow=" + pageCount + "&id=" + temp_person_id
									+ "> 【尾页】</a>");
						}
						//显示当前页的前三页与后三页
						for (i = ((pageNow - 3) > 0 ? (pageNow - 3) : 1); i <= (pageCount < (pageNow + 3)
								? pageCount
								: (pageNow + 3)); i++) {
							out.println(" <a   href=showParking_personOneDetailed.jsp?pageNow=" + i + "&id=" + temp_person_id + ">["
									+ i + "]</a>");
						}
					}
					out.print("第");
		...
		<script type="text/javascript">
			...
			function jump(maxPage) {
				...
				if (pages_int > 0 && pages_int <= maxPage_int) {
					//alert("bbbb"+pages)
					window.location.href = "showParking_personOneDetailed.jsp?pageNow="
